...
public class MainAssembler {
    ...
    public static ArrayList<MainAssembler.RegisterNode> registerInfo = new ArrayList<>();

    public static List<Integer> gsr = new LinkedList<>();          //Global Shift Register
    static int gsrSize = 2;                                      //GSR Size
    public static ArrayList<String> tableOfPredictions = new ArrayList<>();
    ...
    public static void initialisePredictionTable(){
        for (int i = 0 ; i < (Math.pow(2,gsrSize)) ; i++){
            tableOfPredictions.add("00");
        }
    }

    public static void initialiseGSR(){
        for (int i = 0 ; i < gsrSize ; i++){
            gsr.add(0);
        }
    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        // takes 3rd argument at cmd line as gsrSize, if it exists
        if (args.length > 2){
            gsrSize = Integer.parseInt(args[2]);
        }
        // try setting gsrSize to args[1]
        //      1. if it goes through, then there is NO script file
        //      2. if it does not go through, then there IS a script file
        // if script file is used, set scriptFlag and read script into string array
        if (args.length > 1){                           //BUG EXSISTS HERE
            try{
                gsrSize = Integer.parseInt(args[1]);
            }
            catch(Exception e){
    ...
    public static void showBranchAccuracy(){
        System.out.println();
    ...
    public static void updateGSR(int taken){
        gsr.remove(0);
        gsr.add(taken);
    }

    public static int findTableIndex(){
        String s;
        String binary = "";
        for (int i = 0 ; i < gsrSize ; i++){
            if (gsr.get(i) == 0){
        ...
        return ALU.binaryToDecimal(binary, false);
    }

    public static int branchPrediction(){
        int prediction;
        int index = findTableIndex();
        String twoBit = tableOfPredictions.get(index);
        if (twoBit.equals("00") || twoBit.equals("01")){
            prediction = 0;
        }
        else{
            prediction = 1;
    ...
    public static void updateTableOfPredictions(int prediction, int actual){
        totalPredictions++;
        int index = findTableIndex();
        if (prediction == actual){
            correctPredictions++;
            if (prediction == 1){
                tableOfPredictions.set(index, incrementTwoBit(tableOfPredictions.get(index)));
            }
            else{
                tableOfPredictions.set(index, decrementTwoBit(tableOfPredictions.get(index)));
            }
        }
    ...
    public static String incrementTwoBit(String a){
        if (a.equals("00")){
            return "01";
        }
        else if (a.equals("01")){
    ...
    public static String decrementTwoBit(String a){
        if (a.equals("11")){
            return "10";
        }
        else if (a.equals("10")){
    ...
    public static void operateI(String[] str){
        ...
        switch(opcode){
            case "001000":
                ...
                break;
            case "000101":
                int prediction = branchPrediction();
                //System.out.println("prediction:" + prediction);
                int[] temp = ALU.bne(rs,rt,immediate,pc, registerInfo);
                pc = temp[0];
                int taken = temp[1];
                updateTableOfPredictions(prediction, taken);
                updateGSR(taken);
                break;
            case "000100":
                int prediction2 = branchPrediction();
                int[] temp2 = ALU.beq(rs,rt,immediate,pc,registerInfo);
                pc = temp2[0];
                int taken2 = temp2[1];
                updateTableOfPredictions(prediction2, taken2);
                updateGSR(taken2);
                break;
            case "100011":
